...
namespace Microcharts.Samples.Forms
{
    public partial class MainPage : Xamarin.Forms.ContentPage
    {
        ...
        private void ChangeChart(object sender, EventArgs e)
        {
            ...
            var newChart = Activator.CreateInstance(type) as Chart;
            if (type == typeof(LineChart) || type == typeof(PointChart))
            {
                ((PointChart)newChart).ShowYAxisText = true;
            }

            this.chart.Chart = newChart;
